...
// Website API + D1 + GitHub Media + Telegram Bot + Auto Telegram Posting
...
function getTelegramGroupIds(env) {
...
async function ensureSchema(env) {
  ...
  const productColumns = await env.DB
    .prepare("PRAGMA table_info(products)")
    .all();

  const columns = productColumns.results || [];
  ...
  if (!columns.some(c => c.name === "featured")) {
    await env.DB.prepare(
      "ALTER TABLE products ADD COLUMN featured INTEGER NOT NULL DEFAULT 0"
    ).run();
  }

  const telegramColumns = await env.DB
    .prepare("PRAGMA table_info(telegram_sessions)")
    .all();

  if (
    !(telegramColumns.results || []).some(
      c => c.name === "admin_unlocked"
    )
  ) {
    await env.DB.prepare(
...
async function getTelegramFile(fileId, env, type = "image") {
...
async function saveTelegramMedia(fileId, env, type) {
  ...
  const uploaded = await uploadToGitHub(file, env);
...
async function getTelegramSession(chatId, env) {
  return env.DB
    .prepare(
...
async function createTelegramSession(chatId, env) {
  await env.DB
    .prepare(`
...
async function updateTelegramSession(chatId, fields, env) {
  const allowed = [
    "step",
    "title",
    "description",
  ...
  await env.DB
    .prepare(`
      ...
      WHERE chat_id = ?
    `)
    .bind(...values, String(chatId))
    .run();
}

async function clearTelegramSession(chatId, env) {
  await env.DB
    .prepare(
...
async function telegramIsUnlocked(chatId, env) {
...
async function unlockTelegramAdmin(chatId, env) {
  const existing = await getTelegramSession(
    chatId,
    env
  );
...
function productTelegramText(product) {
  return (
    `🛍️ ${product.title}\n\n` +
    `${product.description || ""}\n\n` +
...
async function postProductToTelegramGroups(product, env) {
  const groups = getTelegramGroupIds(env);

  if (!groups.length) {
    return {
      ok: false,
      posted: 0,
      message: "TELEGRAM_GROUP_IDS is not configured."
    };
  }
  ...
  const text = productTelegramText(product);
...
async function createProductFromTelegramSession(chatId, env) {
  if (!(await telegramIsUnlocked(chatId, env))) {
    throw new Error("Telegram admin access is locked.");
  }
  ...
  const result = await env.DB
    ...
    .bind(
      ...
      cleanCategory(session.category),
      images[0],
      images[1],
      images[2],
      images[3],
  ...
  try {
    ...
    telegramResult = {
      ok: false,
      error: error?.message || "Telegram posting failed."
    };
  }
...
async function sendTelegramProductConfirmation(chatId, env) {
  const session = await getTelegramSession(
    chatId,
    env
  );
...
async function handleTelegramUpdate(update, env) {
  ...
  if (
    ...
    if (
      ...
      text === String(env.TELEGRAM_ADMIN_CODE)
    ) {
  ...
  if (session.step === "affiliate_url") {
    ...
    try {
      affiliateUrl = new URL(text).toString();
    } catch {
      await telegramSendMessage(
        env,
        chatId,
        "❌ Invalid URL.\n\n" +
        "Send the complete affiliate URL."
      );
      return;
    }
  ...
  if (session.step === "category") {
    ...
    await telegramSendMessage(
      env,
      chatId,
      "📸 Send product image(s).\n\n" +
      "Up to 5 photos.\n" +
      "Send /done when finished.\n" +
      "Send /skip for no images.",
      removeKeyboard()
    );
    ...
    if (message.video?.file_id) {
      try {
        ...
        await telegramSendMessage(
          ...
          "Send /skip to continue without video."
        );
      }
  ...
  if (session.step === "confirm") {
    if (text === "/publish") {
      ...
      try {
        ...
        const productUrl =
          `${SITE_URL}/?product=${result.id}`;

        const posted =
          result.telegramResult?.posted || 0;

        const total =
          result.telegramResult?.total || 0;
        ...
        await telegramSendMessage(
          ...
          `Website:\n${productUrl}\n\n` +
          `📢 Telegram posted: ${posted}/${total} groups\n\n` +
          "🔒 Admin access locked.\n" +
          "Use /admin for another product.",
          removeKeyboard()
        );
    ...
    await telegramSendMessage(
      env,
      chatId,
      "Send /publish to publish or /cancel to cancel."
    );
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          setupKey !== env.TELEGRAM_SETUP_SECRET
        ) {
          return json(
            {
              ok: false,
