 ...
 *
 * Website API + D1 + GitHub media + Telegram bot
 *
 * Telegram behavior:
 * - Admin/product workflow ONLY works in private chat.
 * - Groups ignore normal messages.
 * - Groups only respond to /getid.
 * - Product publishing still auto-posts to configured groups.
 */
...
function telegramCommand(value) {
...
function githubApiBase(env) {
  ...
  if (!owner || !repo) {
    throw new Error("GITHUB_OWNER or GITHUB_REPO is missing");
  }
...
async function githubDelete(env, rawUrl) {
  const branch = cleanText(env.GITHUB_BRANCH) || "main";
  ...
  if (!getResponse.ok) {
    throw new Error(`GitHub lookup failed: ${getResponse.status}`);
  }
...
async function downloadTelegramFile(env, fileId, kind) {
...
async function verifyAdminSession(request, env) {
  const token =
    cookieValue(request, "zilnet_admin");
...
function readProductFields(body) {
  ...
  const fields = {
    ...
    description:
      cleanText(body.description, 100000),
...
async function handleTelegramCallback(update, env) {
  ...
  /*
   * Category buttons are only valid in PRIVATE chats.
   */
  if (
...
async function handleTelegramUpdate(update, env) {
   ...
   *
   * The bot does NOT create a session here.
   * The bot does NOT ask for the admin code here.
   * The bot does NOT respond to normal group messages.
   *
   * Only /getid works in a group.
   */
   ...
   * Only reply when an actual command is sent.
   */
      ...
      try {
        ...
        await telegramApi(
          ...
          "sendMessage",
          {
            chat_id: chatId,
            text:
              `Image upload failed: ${String(error.message || error)}`
          }
        );
      }
      ...
      try {
        ...
        await telegramApi(
          ...
          "sendMessage",
          {
            chat_id: chatId,
            text:
              `Video upload failed: ${String(error.message || error)}`
          }
        );
      }
  ...
  if (
    ...
    try {
      ...
      const failed =
        created.telegramResult.failures?.length || 0;
      ...
      await telegramApi(
        ...
        "sendMessage",
        {
          chat_id: chatId,
          text:
            `✅ PRODUCT PUBLISHED\n\nProduct ID: ${created.product.id}\n\nWebsite:\n${productShareUrl(created.product.id)}\n\n📢 Telegram posted: ${created.telegramResult.sent}/${groups.length} groups${failed ? `\n⚠️ Failed group posts: ${failed}` : ""}\n\n🔓 Admin access remains unlocked.\nUse /addproduct for another product.`
        }
      );
    } catch (error) {
      await telegramApi(
  ...
  /*
   * Unknown command while unlocked.
   */
  ...
  if (command) {
    await telegramApi(
      ...
      "sendMessage",
      {
        chat_id: chatId,
        text:
          "Unknown command.\n\nUse /addproduct to add a product or /lock to lock admin access."
      }
    );
  }
}
