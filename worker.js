...
async function ensureSchema(env) {
  ...
  const hasFeatured = (columns.results || []).some(
...
async function createTelegramSession(chatId, env) {
  await env.DB
    .prepare(`
      ...
      VALUES (?, 'title', '', '', '', 'Other', '', '', '', '', '', '', CURRENT_TIMESTAMP)
      ON CONFLICT(chat_id)
      DO UPDATE SET
        step = 'title',
        title = '',
...
async function createProductFromTelegramSession(
  ...
  await clearTelegramSession(chatId, env);
...
async function handleTelegramUpdate(update, env) {
  ...
  if (text === "/start") {
    await telegramSendMessage(
      ...
      "Use /addproduct to create a real product on your website.\n\n" +
  ...
  if (session.step === "confirm") {
    ...
    if (text === "/publish") {
      try {
        ...
        const productUrl =
          `${new URL(env.ASSETS ? "https://shopperssuggestions.online" : "https://shopperssuggestions.online").origin}/?product=${productId}`;
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (
        ...
        const result =
          await setupTelegramWebhook(env);
      ...
      if (
        ...
        const featuredParam =
          (url.searchParams.get("featured") || "").trim();
